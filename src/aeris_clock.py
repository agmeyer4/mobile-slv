...
The correction = median(Epoch_time - epoch(Time Stamp)) is computed per logger file,
surveyed across all files to confirm per-deployment variation, then applied to the
corresponding Aeris Raw and Spectra files.
"""
...
def apply_offset_to_raw(in_path, offset_s, out_path):
    ...
    Writes corrected file to out_path, creating parent directories as needed.
    All non-timestamp columns are unchanged.
    """
...
def apply_offset_to_spectra(in_path, offset_s, out_path):
    ...
    Writes corrected file to out_path, creating parent directories as needed.
    All spectral columns are unchanged.
