...
- **`03_instrument_aligned/quality_manifest.yaml`** — the human quality survey. It lives inside
  a stage output directory and is the only artifact the pipeline cannot rebuild by running
  something. Losing it means re-reviewing every session by hand.
